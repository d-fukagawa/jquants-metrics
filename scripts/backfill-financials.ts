 ...
 *   EDINETDB_API_KEY     EDINETDB API キー（details 同期時は必須）
 *   EDINET_API_KEY       公式EDINET API キー（details 同期時は必須）
...
import { syncFinancialSummary, syncFinsDetails, syncFinsDetailsFromEdinet, type DetailsSource } from '../src/services/syncService'

const databaseUrl = process.env.DATABASE_URL
const apiKey = process.env.JQUANTS_API_KEY
const edinetDbApiKey = process.env.EDINETDB_API_KEY
...
for (const { code } of target) {
  ...
  if (includeDetails) {
    if (detailsMode === 'jquants') {
      try {
        const nDetails = await withRetry(() => syncFinsDetails(db, apiKey, code), retryPerCode)
        detailsCount += nDetails
        if (nDetails > 0) detailsSources.jquants += nDetails
      } catch (e) {
        if (isSubscriptionError(e) && fallbackOn403 && edinetDbApiKey && edinetApiKey) {
          detailsMode = 'edinet'
          const message = e instanceof Error ? e.message : String(e)
          console.warn(`[fin-backfill] details fallback enabled after 403: ${message}`)
    ...
    if (includeDetails && detailsMode === 'edinet' && edinetDbApiKey) {
      const out = await withRetry(
        () => syncFinsDetailsFromEdinet(db, edinetDbApiKey, edinetApiKey ?? null, code),
        retryPerCode,
      )
...
const summaryDetailsSource: DetailsSource =
  detailsSources['edinet+official'] > 0
    ? 'edinet+official'
    : detailsSources.edinetdb > 0
      ? 'edinetdb'
      : 'jquants'
