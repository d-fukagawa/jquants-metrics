...
describe('getSyncStatusSummary', () => {
  ...
  it('returns aggregated sync stats', async () => {
    const execute = vi.fn()
      ...
      .mockResolvedValueOnce({ rows: [{ total_count: 350000, date_count: 90, code_count: 4180, latest_date: '2026-03-11' }] })
      .mockResolvedValueOnce({ rows: [{ latest_count: 4180 }] })
    ...
    expect(s.valuationTotalCount).toBe(350000)
    expect(s.valuationLatestDate).toBe('2026-03-11')
    expect(s.valuationLatestDateCount).toBe(4180)
  ...
  it('handles empty tables', async () => {
    const execute = vi.fn()
      ...
      .mockResolvedValueOnce({ rows: [{ total_count: 0, date_count: 0, code_count: 0, latest_date: null }] })
      .mockResolvedValueOnce({ rows: [{ latest_count: 0 }] })
    ...
    expect(s.valuationLatestDate).toBeNull()
